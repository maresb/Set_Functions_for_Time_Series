...
def get_delta_t(times, measurements, measurement_indicators):
    ...
    dt_array = tf.TensorArray(tf.float32, tf.shape(input=measurement_indicators)[0])
    # First observation has dt = 0
    first_dt = tf.zeros(tf.shape(input=measurement_indicators)[1:])
    dt_array = dt_array.write(0, first_dt)

    def compute_dt_timestep(i, last_dt, dt_array):
        last_dt = tf.compat.v1.where(
            measurement_indicators[i-1],
            tf.fill(tf.shape(input=last_dt), tf.squeeze(times[i] - times[i-1])),
            times[i] - times[i-1] + last_dt
        )
        dt_array = dt_array.write(i, last_dt)
        return i+1, last_dt, dt_array

    n_observations = tf.shape(input=scattered_times)[0]
    _, last_dt, dt_array = tf.while_loop(
        cond=lambda i, a, b: i < n_observations,
        body=compute_dt_timestep,
